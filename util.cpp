...
void Util::storeNewResult(vector<double> *new_pubMatchtime,int pubs,vector<double> *new_subMatchtime,int subs)
{
	...
	for (int i=0; i<pubs; ++i)
	{
		fileStream.open(string("../newpubDetail/") + Util::Int2String(i+1) + string(".txt"), ios::app);    //what's the key of Util::Int2String?
...
double Util::Average(const vector<double> &a)            //double
{
    double sum = 0;
...
double Util::Sum(const vector<double>& a) {
	double sum = 0;
	for (int i = 0; i < a.size(); i++)
		sum += a[i];
	return sum;
...
int Util::Average(const vector<int> &a)                  //int
{
    int sum = 0;
    for (int i = 0; i < a.size(); i ++)
        sum += a[i];
...
std::string Util::RemoveLastZero(std::string str) // why do we remove the last zero?
{
	int strLen = str.size();
...
void Util::WriteData2(string fileName, string text)
{
	ofstream fileStream;
...
string Util::Double2String(double value)
{
	string result;
	char buffer[200];
	sprintf(buffer, "%lf ", value);   //sprintf 
